...
# ============================== HELP ========================================
# ============================================================================
# Help : ./check_snmp_asterisk_calls-active.pl --help
#
# ============================================================================
...
# System description
my $asterisk_calls_active     	= "1.3.6.1.4.1.22736.1.2.5.0"; 	# SNMP uptime
...
sub p_version {
	print "check_snmp_asterisk_calls-active version : $Version\n";
}
...
-C, --community=COMMUNITY NAME
   Community name for the hosts SNMP agent
-1, --v1
   Use SNMPv1
-2, --v2c
   Use SNMPv2c (default)
...
@temp_oid=($asterisk_calls_active);
$result_t = $session->get_request( Varbindlist => \@temp_oid);
my $active_calls = $$result_t{$asterisk_calls_active};

# Clear the SNMP Transport Domain and any errors associated with the object.
$session->close;

#if (defined($active_calls && looks_like_number($active_calls))){
...
if (defined($active_calls) && looks_like_number($active_calls)){
	...
	$output = "Number of active calls: ".$active_calls;

}else{
	$final_status = 2;
	$output = "Can't get SNMP call data";
}
